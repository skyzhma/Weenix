...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        int t = file->vn_cdev->cd_ops->read(file->vn_cdev, offset, buf, count);
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        int t = file->vn_cdev->cd_ops->write(file->vn_cdev, offset, buf, count);
...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
        ...
        int t = file->vn_cdev->cd_ops->mmap(file, vma, ret);
...
special_file_fillpage(vnode_t *file, off_t offset, void *pagebuf)
{
        ...
        int t = file->vn_cdev->cd_ops->fillpage(file, offset, pagebuf);
...
{   
        ...
        int t = file->vn_cdev->cd_ops->dirtypage(file, offset);
...
special_file_cleanpage(vnode_t *file, off_t offset, void *pagebuf)
{
        ...
        int t = file->vn_cdev->cd_ops->cleanpage(file, offset, pagebuf);
