...
do_link(const char *from, const char *to)
{
        ...
        size_t namelenTo;
        const char *nameTo = NULL;

        int t = open_namev(from, 0, &vfrom, NULL);
        if (t == -ENOTDIR || t == -ENOENT || t == -ENAMETOOLONG) return t;
        if (vfrom->vn_ops->lookup) {
            vput(vfrom);
            return -EPERM;
        }
        ...
        // t = lookup(vto, nameTo, namelenTo, &temp);
        // if (t != -ENOENT) {
        //     vput(temp);
        //     vput(vfrom);
        //     vput(vto);
        ...
        t = vto->vn_ops->link(vfrom, vto, nameTo, namelenTo);
