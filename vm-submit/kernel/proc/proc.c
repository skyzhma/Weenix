...
int
isChildOfProc(proc_t *proc,  pid_t pid) 
{
        proc_t *p;
...
proc_t *
deadChildProc(pid_t pid) 
{
        proc_t *p;
        list_iterate_begin(&curproc->p_children, p, proc_t, p_child_link) {
                if (p->p_state == PROC_DEAD && (pid == -1 || p->p_pid == pid)) {
                        dbg(DBG_PRINT, "(GRADING1C)\n");
                        return p;
                }
        } list_iterate_end();
...
proc_kill_all()
{
        ...
        proc_t *proc_idleproc = proc_initproc->p_pproc;

        proc_t *p;
        list_iterate_begin(&_proc_list, p, proc_t, p_list_link) {
                if (p->p_pid != PID_IDLE && p != curproc && !isChildOfProc(proc_idleproc, p->p_pid)) {
                        proc_kill(p, 0);
                        dbg(DBG_PRINT, "(GRADING1C)\n");
                }
        } list_iterate_end();

        if (!isChildOfProc(proc_idleproc, curproc->p_pid)) {
            proc_kill(curproc, 0);
            dbg(DBG_PRINT, "(GRADING1C)\n");
        }
        dbg(DBG_PRINT, "(GRADING1C)\n");
...
do_waitpid(pid_t pid, int options, int *status)
{
        ...
        if (list_empty(&curproc->p_children) || (pid != -1 && !isChildOfProc(curproc,pid))) {
                dbg(DBG_PRINT, "(GRADING1C)\n");
                return -ECHILD;
        }

        while (1) {
            proc_t *p = deadChildProc(pid);
            if (p != NULL) {
