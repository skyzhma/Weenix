 ...
 * user/libc/syscall.c if you're curious.kernel info vmmap_mapping_info curproc->p_vmmap
 *
...
do_brk(void *addr, void **ret)
{
        ...
        uint32_t High = USER_MEM_HIGH;

        if (vma->vma_plink.l_next != &vma->vma_vmmap->vmm_list) {

        	vmarea_t *next_vma = list_item(vma->vma_plink.l_next, vmarea_t, vma_plink);

        	High = (uint32_t)PN_TO_ADDR(next_vma->vma_start);

        }

        if ((uint32_t)addr > High || addr < curproc->p_start_brk) {
                dbg(DBG_PRINT,"(GRADING3E)\n");
                return -ENOMEM;
        }
        
        curproc->p_brk = addr;
        vma->vma_end = ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk));
        *ret = curproc->p_brk;
        dbg(DBG_PRINT, "(GRADING3B)\n");
        return 0;
}
