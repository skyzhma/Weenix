...
handle_pagefault(uintptr_t vaddr, uint32_t cause)
{
        ...
        if (!vma || ((cause & FAULT_WRITE) && !(vma->vma_prot & PROT_WRITE)) || !(vma->vma_prot & PROT_READ)) {
                dbg(DBG_PRINT,"(GRADING3D)\n");
        	do_exit(EFAULT);
        }
        ...
        int t = pframe_lookup(vma->vma_obj, pagenum, cause & FAULT_WRITE, &pf);
        if (t < 0 || !pf) {
                dbg(DBG_PRINT,"(GRADING3D)\n");
        	do_exit(EFAULT);
        } 
        ...
        pt_map(curproc->p_pagedir, (uintptr_t)PAGE_ALIGN_DOWN(vaddr), paddr, pdFlag, ptFlag);
