...
sys_read(read_args_t *arg)
{
        ...
        err = do_read(kern_args.fd, buf, kern_args.nbytes);
        if (err < 0) {
            curthr->kt_errno = -err;
...
sys_write(write_args_t *arg)
{
        ...
        err = copy_from_user(buf, kern_args.buf, kern_args.nbytes);
...
sys_getdents(getdents_args_t *arg)
{
        ...
        tmp = 0;
        count = kern_args.count;
        while (tmp < count) {

            err = do_getdent(kern_args.fd, kern_args.dirp);
            if (err < 0) {
                curthr->kt_errno = -err;
                dbg(DBG_PRINT, "(GRADING3B)\n");
                dbg(DBG_PRINT, "(GRADING3D)\n");
            ...
            if (err == 0) break;
            copy_to_user(arg->dirp+tmp, kern_args.dirp, err);
            tmp += err;
        }
